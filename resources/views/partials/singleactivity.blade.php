     ...
     <?php
      ...
      foreach($allActivity as $singleActivity){
        ...
        if($olddate === $newdate){
            ...
            $content .='<p><img src="'.asset('storage/profile/'.$singleActivity->user_id.'_profile_thumb.jpg').'" alt="user" class="img-responsive profile-photo profile-photo-md"><a href="'.url('profile/'.Auth::id()).'">'.Auth::user()->name.'</a> Create a new <a href="'.url('post/'.$singleActivity->post_id).'">post</a>.</p>';
        }
        else{
            $content .="</div></div>";
            $content .="<div class='card my-3'><div class='card-header'>".$newdate."</div><div class='card-body'>";
